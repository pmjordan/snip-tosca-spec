...
* `<foo_*>` is used to refer to any or all of the placeholders in the sequence
  or map notations.
...
- `<capability_def_*>`: represents the optional map of capability
  definitions for the node type.

- `<requirement_def_*>`: represents the optional list of requirement
...
a result supports the common keynames listed in
[the section Common Keynames in Type Definitions](#common-keynames-in-type-definitions).
In addition, the capability type definition has the following recognized keynames:
...
<capability_type_name>:
  ...
  valid_relationship_types: [ <relationship_type_name_1>, <relationship_type_name_2>, ... ]
```
...
- `<node_type_name_*>`: represents the optional list of one or more type
...
- `<relationship_type_name_*>`: represents the optional list of one or more
...
- `<directive_*>`: represents the optional list of strings that
...
- `<interface_refinement_*>`: represents refinements for one or more already
...
- `<property_assignment_*>`: within the relationship declaration, it
  represents the optional map of property assignments for the declared
  relationship.

- `<attribute_assignment_*>`: within the relationship declaration, it
...
- `<allocation_property_assignment_*>`: within the allocation declaration, it
...
"my-application" of type "WebApplication" that declares
...
The "host" requirement also specifies a capability type of "Container" to
...
"Endpoint.Database" in a node template called `my-database`. However,
...
- If a node template does not define an explicit requirement
  ...
  - Additionally, the `<count_value>` is assumed to be equal to the
    `<min_count>` value of the requirement definition in the corresponding
    node type.
...
assignments (optional as well as non-optional) must be less than or equal
...
whether the requirement were non-optional or optional. All that matters
...
if both the capacity allocations for "num-cpu" and "mem-size" are
...
So assuming that "num-cpu" property in the target capability of a
candidate node has value 4 and the sum of capacity allocations of the
other fulfilled requirements to that capability for "num-cpu" is 1 then
...
Another node with "num-cpu" with value 2 could not be a valid target since
...
- Within a node template definition using the "select" directive:
  ...
  - Note that the context within which the node filter is evaluated is the
    ...
    value in any TOSCA Path expressions refers to the node.
...
- Within requirement definitions or assignments:
  ...
  - Note that the context within which the node filter is evaluated
    ...
    value in any TOSCA Path expressions refers to the relationship.
...
support a specific range of CPUs (i.e., "num-cpus" value between 1 and 4)
...
sequences as other types ("!!bool", "!!int", "!!float", etc.) *before*
...
4.  Relatedly, although in YAML 1.2 a "!!string" is already defined as a
    Unicode sequence See [\[YAML-1.2\] section
    ...
    encoding of the YAML stream. See [\[YAML-1.2\] chapter
...
1. The JSON schema in [\[YAML-1.2\] chapter
...
float except where the int is supplied as the value of a TOSCA
property of type float. Type conversion in this exceptional case
is to prevent the need for users to add a ".0" suffix to literal
integers that must be floats. 
...
A TOSCA *bytes* is an array of arbitrary bytes. Because we adhere to
...
help you convert arbitrary data to Base64.
...
3. TOSCA bytes values *cannot* be the null value but *can* be empty
   arrays (a bytes value with length zero). See [`nil`](#nil), below.

#### 9.1.1.6 `nil` <a name=nil></a>

The TOSCA *nil* type always has the same singleton value. No other type
can have this value.
...
- The TOSCA functions `$equal`, `$greater_than`, `$greater_or_equal`,
  `$less_than`, and `$less_or_equal` all use the *universal* instant,
  i.e. as the local instant is converted to UTC by applying the
  timezone offset.
...
#### 9.1.2.2 `scalar` <a name=scalar></a>

The TOSCA *scalar* types can be used to define scalar values
...
- `<number>`: is a mandatory number in YAML notation preceded by at whitespace. The
   number may be in any valid YAML format including hex, octal and scientific as well as
   float and integer.

- `<unit>`: is a mandatory string preceeded by at least one white space. The string
  represents the name of unit in which the scalar is measured.
...
A scalar type definition is a type of TOSCA type definition and as a result supports the common keynames listed in
[the section Common Keynames in Type Definitions](#common-keynames-in-type-definitions) In addition, the scalar
type definition has the following recognized keynames:
...
- `<unit>`: The YAML string, with no white space, of the unit element of the scalar.

- `<multiplier>`: An integer or float which which MUST be used by a TOSCA processor to convert values with this unit into values in the canonical unit.
...
The TOSCA *list* type allows for specifying multiple values.
For example, if an application allows for being
...
- `<list_entry_*>`: represents an element of the list.
...
```yaml
{ <entry_key_1>: <entry_value_1>, <entry_key_2>: <entry_value_2>, ... }
```
...
- `<property_def_*>`: represents the optional map of one or more
  property definitions that provide the schema for the data type

  - `<property_def_*>` are only allowed for complex type definitions
    and MAY NOT be added to custom data types `derived_from` TOSCA
    built-in types.

- `<key_schema_def>`: if the data type derives from the TOSCA `map`
...
- `<entry_schema_def>`: if the data type derives from the TOSCA `map`
  or `list` types (i.e. `<existing_type>` name is a `list` or `map` or derives
  from a `list` or `map`), it represents the mandatory schema definition for
...
`list` or `map` type or in parameter, property, or attribute definitions of
a `list` or `map` type.
...
- `<key_schema_def>`: if the `<schema_type>` is `map`, it represents the
  optional schema definition for the keys of that map's entries.

- `<entry_schema_def>`: if the `<schema_type>` is `list` or `map`, it
...
|`key_schema`|conditional|schema definition|The schema definition for the keys used to identify entries in properties of type `map` (or types that derive from `map`). If not specified, the `key_schema` defaults to string. For properties of type other than map, the `key_schema` is not allowed. |
|`entry_schema`|conditional|schema definition|The schema definition for the entries in properties of collection types such as `list`, `map`, or types that derive from `list` or `map`. If the property type is a collection type, `entry_schema` is mandatory. For other types, the `entry_schema` is not allowed.|
...
- `<default_value>`: contains a type-compatible value that is used as a
  ...
  fixed value definition or via property assignment);
...
- `<key_schema_def>`: if the `<property_type>` is `map` or derives from
  `map`, represents the optional schema definition for the keys used to
  identify entries in that map.

- `<entry_schema_def>`: if the `<property_type>` is `list` or `map`, or
  derives from `list` or `map`, represents the mandatory schema definition
...
|`entry_schema`|conditional|schema definition|The schema definition for the entries in attributes of collection types such as `list`, `map`, or types that derive from `list` or `map`) If the attribute type is a collection type, `entry_schema` is mandatory. For other types, the `entry_schema` is not allowed.|
...
- `<key_schema_def>`: if the `<attribute_type>` is `map`, represents the
  optional schema definition for the keys used to identify entries in
  that map.

- `<entry_schema_def>`: if the `<attribute_type>` is `list` or `map`,
  represents the mandatory schema definition for the entries in that map
  or list.
...
- `<key_schema_def>`: if the `<parameter_type>` is `map`, represents the
  optional schema definition for the keys used to identify entries in
  that map. Note that if the `key_schema` is not defined, the `key_schema`
  defaults to string.

- `<entry_schema_def>`: if the `<parameter_type>` is `list` or `map`,
  represents the mandatory schema definition for the entries in that map
  or list.

- `<attribute_selection_format>`: a list that corresponds to a valid
  attribute selection format; the parameter is mapped onto an attribute
...
A *parameter value assignment* is used to assign a value to an
...
```yaml
<parameter_name>: [ <tosca_path>, <attribute_name>, <nested_attribute_name_or_index_or_key_1>, <nested_attribute_name_or_index_or_key_2>, ... ]
```
...
- `<tosca_path>`: used by the TOSCA
  processor to traverse the representation graph to reach the
...
value, and the subsequent "configure" operation could then update that
...
```yaml
$get_property: [ <tosca_path>, <property_name>, <nested_property_name_or_index_1>, <nested_property_name_or_index_2>, ... ]
```
...
```yaml
$get_attribute: [<tosca_path, <attribute_name>, <nested_attribute_name_or_index_1>, <nested_attribute_name_or_index_2>, ... ]
```
...
function, except with reversed argument order.
...
$join: [ [ <string_1>, <string_2>, ... ], <delimiter> ]
```
...
```yaml
$sum: [ <int_float_or_scalar_type_1>, <int_float_or_scalar_type_2>, ... ]
```
...
```yaml
$difference: [ <int_float_scalar_type_1>, <int_float_scalar_type_2> ]
```
...
Each `<signature_def_*>` uses the following grammar:
...
- The `variadic` keyname defines if the last defined argument may be
  ...
  - If `variadic` is true, the last defined argument may be repeated any
    ...
    - If `optional_arguments` is defined, then the last defined argument
      is the last defined optional argument. Note that in this case we
...
- `<parameter_def_*>`: represents the optional map of parameter
...
- `<operation_def_*>`: represents the optional map of one or more
  operation definitions.

- `<notification_def_*>`: represents the optional map of one or more
...
- `<parameter_def_or_refinement_*>`: represents the optional map of
...
- `<operation_refinement_*>`: represents the optional map of operation
  definition refinements for this interface; the referred operations
  must have been previously defined in the interface type.

- `<notification_refinement_*>`: represents the optional map of notification
...
- `<parameter_value_assignment_*>`: represents the optional map of parameter
...
- `<operation_assignment_*>`: represents the optional map of operation
...
```yaml
<operation_name>: <operation_implementation_def>
```
...
- `<operation_implementation_def>`: represents the optional specification of the operation's implementation.

- `<parameter_def_*>`: represents the optional map of parameter
  definitions which the TOSCA orchestrator will make available as inputs to or
  receive as outputs from the corresponding implementation artifact during its execution.
...
- Defining a fixed value for an input parameter (as part of its
  ...
  context of an interface type definition functions such as `$get_property`
...
- `<operation_implementation_def>`: represents the optional
...
- `<parameter_mapping_assignment_*>`: represents the optional map of
...
- The behavior for implementation of operations SHALL be overwrite. That
...
operations. Artifacts registered to recieve events may be configured
by means of parameters provided under the `inputs` keyname of the
...
```yaml
<notification_name>: <notification_implementation_def> 
```
...
- `<notification_implementation_def>`: represents the optional
...
- `<primary_artifact_def>`: represents a full inline definition of
...
- `<dependent_artifact_*>`: represents the optional ordered list of
...
a result supports the common keynames listed in
[the section Common Keynames in Type Definitions](#common-keynames-in-type-definitions). In addition, the
...
- `<file_extension_*>`: represents the optional list of one or more
...
- Selection of artifact processor. This category includes informational
  ...
  in TOSCA as keynames for the artifact.
...
- `<property_assignment_*>`: represents an optional map of property assignments
...
- `<operation_implementation_def>`: represents a full inline
  definition of an implementation artifact

- `<attribute_mapping_*>`: represents the optional map of attribute_mappings
...
- `<activity_def_*>`: represents a list of activity
  definitions.
...
- `<parameter_assignment_*>`: represents the optional map of parameter
...
`count` keyname that specifies the requested number of runtime
representations for this node template. This `count` keyname is
similar to the `count` keyname in requirement definitions.
...
- The target capability in each target node defines a property that is
  ...
  defines a "target-count" property. The value of this property is set
...
- The requirement in each source node includes an `allocations`
  ...
  allocates a single unit from the "target-count" property in the
  target capability.
...
- `<property_mapping_*>`: represents the map of *property to input* mappings.

- `<attribute_mapping_*>`: represents the map of *output to attribute*
  mappings.

- `<capability_mapping_*>`: represents the map of capability mappings.

- `<requirement_mapping_*>`: represents the list of requirement
  mappings.

- `<interface_mapping_*>`: represents the map of interface mappings.
...
- `<idx>`: index of the relationship defined from that requirement (0 is the
  ...
  if the value ALL is used by as index, the corresponding
...
- `<idx>`: index of the relationship defined from that requirement (0 is the
  first index); if the index is missing, index 0 is assumed;
  if the value ALL is used by as index, all attributes
...
YAML sequences rather than maps. In addition, each of the mappings in
the sequences may in turn identify a sequence of requirements.
...
map requirements onto non-optional requirements in the substituting
...
A *group* is a logical grouping of nodes for purposes of uniform
...
<group_type_name>:
  ...
  attributes:
    ...
    <attribute_def_2>
    ...
  members: [ <valid_member_type_1>, <valid_member_type_1>, ... ] 
```
...
- `<valid_member_type_*>`: represents the optional list of TOSCA Node
...
<group_name>:
  ...
  attributes:
    ...
    <attribute_assignment_2>
    ...
  members: [ <node_template_1>, <node_template_2>, ... ]
```
...
- `<attribute_assigment_*>`: represents the optional map of attribute
  assignments for the group definition that provide values for
  attributes defined in its declared group type.

- `<node_template_*>`: contains the mandatory list of one or more
  ...
  - if the `members` keyname was defined (by specifying a
    `<valid_member_type_*>`) in the group type of this group then the
...
- `<valid_target_type_*>`: represents the optional list of TOSCA
...
- `<trigger_def_*>`: represents the optional map of trigger
...
- `<policy_target_*>`: represents the optional list of names of node
  templates or groups that the policy is to applied to.

  - if the `targets` keyname was defined (by specifying a
    `<valid_target_type_*>`) in the policy type of this policy then the
...
- `<activity_def_*>`: represents the list of activities that
...
Each keyname/value pair is in a separate line. A list of related keyname/value
pairs, i.e. a list of consecutive keyname/value pairs is called a block.
...
The keyname/value pairs are as follows:
...
`imports` keynames. Note also that artifact files (e.g. scripts,
...
of keynames prefixed with "TOSCA" and "tosca". It is recommended as a good
